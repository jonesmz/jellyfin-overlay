...
# The .NET server only; the web UI is media-video/jellyfin-web.
...
# The web UI is provided separately by media-video/jellyfin-web (installed at
# /usr/share/jellyfin-web, which the service/init files pass via --webdir).
RDEPEND="
	acct-group/jellyfin
	acct-user/jellyfin
	dev-libs/icu
	...
	virtual/zlib:=
"
# acct-* are needed at install time for the fowners calls in src_install;
# virtual/dotnet-sdk comes from dotnet-pkg.eclass.
DEPEND="
	acct-group/jellyfin
	acct-user/jellyfin
"
...
src_unpack() {
	# Only the source tarball; the nuget deps are consumed from DISTDIR.
...
src_install() {
	...
	# Service scripts are named "jellyfin" regardless of the package name.
...
pkg_postinst() {
	...
	elog "The web UI is provided by media-video/jellyfin-web at"
	elog "/usr/share/jellyfin-web (the service files point --webdir there)."
	elog
